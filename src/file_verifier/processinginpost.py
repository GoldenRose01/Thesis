...
def load_dataset_names(file_path):
    if not os.path.exists(file_path):
        print(f"Dataset names file not found: {file_path}")
        return []
...
def generate_namefile(dataset, ruleprefix, type_encoding, selected_evaluation_edit_distance, settings=None):
    if selected_evaluation_edit_distance == 'weighted_edit_distance' and settings:
        return f"{dataset}{ruleprefix}{type_encoding}_evaluation_{selected_evaluation_edit_distance}{int(settings['wtrace_att'])}%,{int(settings['wactivities'])}%,{int(settings['wresource_att'])}%.csv"
    else:
...
def process_single_dataset(file_path, dataset_name, rule_prefix, type_encoding, selected_evaluation_edit_distance,
    ...
    weighted_values = '/'
    if selected_evaluation_edit_distance == 'weighted_edit_distance' and settings:
        weighted_values = f'{int(settings["wtrace_att"] * 100)}% {int(settings["wactivities"] * 100)}% {int(settings["wresource_att"] * 100)}%'
...
def append_to_summary(summary_file, data_row):
    if os.path.exists(summary_file):
        try:
            ...
            summary_df = pd.DataFrame(columns=[
                'dataset_name', 'complexity_type', 'encoding_type', 'weighted_values',
    ...
    summary_df = summary_df.sort_values(
        by=['dataset_name', 'complexity_type', 'encoding_type', 'weighted_values'])
...
def compare_and_delete_if_needed(existing_file_path, new_data_row):
    ...
    if (float(new_data_row[4].rstrip('%')) > existing_avg_fscore * 100 and
            float(new_data_row[5].rstrip('%')) > existing_avg_precision * 100 and
            float(new_data_row[6].rstrip('%')) > existing_avg_recall * 100 and
            float(new_data_row[9].rstrip('%')) > existing_weighted_avg_fscore * 100 and
            float(new_data_row[10].rstrip('%')) > existing_weighted_avg_precision * 100 and
            float(new_data_row[11].rstrip('%')) > existing_weighted_avg_recall * 100):
...
def create_folders(base_folder, rule_prefix, type_encoding, selected_evaluation_edit_distance, settings=None):
    path = os.path.join(base_folder, rule_prefix, type_encoding)
    if selected_evaluation_edit_distance == 'weighted_edit_distance' and settings:
        folder_name = f"{int(settings['wtrace_att'] * 100)}%{int(settings['wactivities'] * 100)}%{int(settings['wresource_att'] * 100)}%"
        path = os.path.join(path, selected_evaluation_edit_distance, folder_name)
    else:
        path = os.path.join(path, selected_evaluation_edit_distance)
...
def move_file_to_folder(file_path, new_folder, file_name):
...
def process_and_update_summary(results_dir, postprocessing_folder, dataset_names):
    ...
    for rule_prefix in rule_prefixes:
        if rule_prefix == '':
            continue
        summary_file_ruleprefix = os.path.join(postprocessing_folder, rule_prefix, f'Summary{rule_prefix}.csv')

        for dataset in dataset_names:
            processed_files = {}
            for (type_encoding, selected_evaluation_edit_distance) in itertools.product(type_encodings,
                                ...
                                if file_name.startswith(
                                    ...
                                    if data_row:
                                        ...
                                        if unique_key in processed_files:
                                            ...
                                            processed_files[unique_key] = file_path
                                            append_to_summary(summary_file_ruleprefix, data_row)
                                            summary_file_type_encoding = os.path.join(postprocessing_folder,
                                                                                      rule_prefix, type_encoding,
                                                                                      f'Summary{type_encoding}.csv')
                                            append_to_summary(summary_file_type_encoding, data_row)
                                            new_folder = create_folders(postprocessing_folder, rule_prefix,
                                                                        type_encoding,
                                                                        selected_evaluation_edit_distance, settings)
                                            move_file_to_folder(file_path, new_folder, file_name)
                            else:
                                ...
                                if file_name == namefile or file_name == namefile.replace(f"{rule_prefix}_", ""):
                                    ...
                                    if data_row:
                                        ...
                                        if unique_key in processed_files:
                                            ...
                                            if rule_prefix == '':
                                                new_file_path = os.path.join(root, f"{normalized_namefile}.csv")
    ...
    print('All summaries have been updated.')
...
if __name__ == "__main__":
    ...
    if dataset_names:
        ...
        print("No dataset names to process.")
