...
def _compute_additional_columns(log, trace_attributes, resource_attributes) -> dict:
    ...
    """
    trace_attrs = [att for att in trace_attributes if att not in ["concept:name", "time:timestamp", "label"]]
...
def _trace_to_row(trace, prefix_length: int, additional_columns, prefix_length_strategy: str, padding, columns: list,
    ...
    # Aggiunta degli attributi delle risorse
    trace_row += [trace.attributes.get(att, 0) for att in additional_columns['resource_attributes']]
